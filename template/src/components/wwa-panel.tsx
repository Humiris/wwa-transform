...
export const WWAPanel = ({
  ...
  const handleSubmit = async (textOverride?: string) => {
    ...
    if (intent === "generate" && onGenerateView) {
      ...
      try {
        ...
        setMessages((prev) => [...prev, {
          id: String(Date.now() + 1),
          role: "assistant",
          content: "I wasn't able to generate that. Could you try rephrasing? For example: 'Create a report about Brand merchant presence in France' or 'Build a dashboard showing card tier benefits'.",
          type: "text",
        }]);
      }
      return;
    ...
    if (isDemoAnswer) {
      ...
      if (currentStep === "companyType") {
        ...
        const productMap: Record<string, string[]> = {
          "Banks & Financial Institution": ["Solution A", "Solution B", "Solution D", "Solution C", "Fraud & Risk Solutions"],
          "Fintech & Technology Partner": ["Developer Platform", "Solution A", "Solution B", "Solution D", "Solution E"],
          "Enterprise Business": ["Commercial Card Programs", "Solution A", "Virtual Card Solutions", "Cross-Border Payments"],
          "Small Business": ["Business Credit Cards", "Solution E", "Solution D", "Solution A"],
          "Government": ["Government Payment Solutions", "Solution A", "Commercial & Government Cards"],
        };
    ...
    if (detectedCardIds.length > 0) {
      ...
      if (combined.includes("visa direct") || combined.includes("push payment") || combined.includes("real-time payment")) detectedSolutionIds.push("solution-a");
      if (combined.includes("token service") || combined.includes("tokenization")) detectedSolutionIds.push("solution-c");
      if (combined.includes("b2b connect") || combined.includes("cross-border")) detectedSolutionIds.push("solution-b");
      if (combined.includes("agentic commerce") || combined.includes("trusted agent")) detectedSolutionIds.push("agentic-commerce");
      if (combined.includes("developer platform") || combined.includes("visa api")) detectedSolutionIds.push("developer");
      if (combined.includes("visa infinite") && !detectedSolutionIds.includes("premium")) detectedSolutionIds.push("premium");
      if (combined.includes("visa signature") && !detectedSolutionIds.length) detectedSolutionIds.push("standard");
    ...
    if (detectedCardIds.length > 0) {
      ...
      if (isCompare && onCompare) {
        ...
        else if (lower.includes("traditional") || lower.includes("tier") || lower.includes("infinite") || lower.includes("signature")) title = "Basic Tier vs Signature vs Infinite";
